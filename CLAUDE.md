...
- Le pont video vers Plotline repose sur trois variables dans `.env.local` : `PLOTLINE_API_KEY` (cle partagee, doit correspondre exactement a `EXTERNAL_VIDEO_JOBS_API_KEY` cote Plotline), `PLOTLINE_PROFILE_ID` (profil Plotline qui RECOIT la video : un dossier de rangement, pas la personne a l'ecran ; anciennement `PLOTLINE_INFLUENCER_ID`, toujours lue en repli ; a pointer vers la marque Sassify `cmton85kd0002i0l1hppn29y9` plutot que vers le persona Jeremie), `PLOTLINE_BASE_URL` (`https://plotline.sassify.fr`). Sans `PLOTLINE_API_KEY` ou `PLOTLINE_PROFILE_ID`, le script video se contente d'un avertissement et continue -- il n'est jamais bloquant pour la publication de l'article.
- Le VPS execute un cron (VERIFIE le 2026-09-21 par `crontab -l`) : `0 9 * * 1` = `build-knowledge-base.cjs` puis `generate-article.cjs` (article ecrit en `brouillon`, notification Telegram). La video n'est PAS lancee par ce cron : `telegram-bot.cjs` (process pm2 `sassify-telegram-bot`) lance `generate-video-script.cjs <slug>` au moment ou l'utilisateur clique "publier" dans Telegram (`traiterArticle`). Sans clic sur "publier", aucune video n'est demandee. Autre point : `VIDEO_SCRIPT_MODE` n'est PAS defini dans le `.env.local` du VPS, donc le mode par defaut `court` (10s) s'applique ; pour des videos de 40s, ajouter `VIDEO_SCRIPT_MODE=long` (apres deploiement de Plotline).
